...
def get_data(url):
    ...
    for item in range(1, 24):
        ...
        # -----Create 'data' folder in file directory first-----
        ...
        # writing data into folder
        with open(f"{folder_name}/projects_{item}.html", "w", encoding="utf-8-sig") as file:
            file.write(req.text)

        with open(f"{folder_name}/projects_{item}.html", encoding="utf-8-sig") as file:
            src = file.read()

        soup = BeautifulSoup(src, "lxml")
        articles = soup.find_all("article", class_="ib19")

        # Saving articles' urls
        projects_urls = []
        ...
        for project_url in projects_urls:
            ...
            # saving file under project name
            with open(f"{folder_name}/{project_name}.html", "w", encoding="utf-8-sig") as file:
                file.write(req.text)

            with open(f"{folder_name}/{project_name}.html", encoding="utf-8-sig") as file:
                src = file.read()

            soup = BeautifulSoup(src, "lxml")
            project_data = soup.find("div", class_="inside")
            
            # collecting project logo
            try:
            ...
            # collecting project short description
            try:
            ...
            # store data to save it later in json
            projects_data_list.append(
                {
                    "Project name": project_name,
                    "Project logo URL": project_logo,
        ...
        iteration_count -= 1
        print(f"Iteration #{item} completed, remaining iterations: #{iteration_count}")
        if iteration_count == 0:
            print("Parsing completed")
        time.sleep(random.randrange(2, 4))

    # Saving data into json
    with open("data/projects_data.json", "a", encoding="utf-8-sig") as file:
